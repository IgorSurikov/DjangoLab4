...
def product_grades(request, product_id):
    ...
    logger.info(log_username(request) + " посетил страницу сортов продукта: " + p.product_name)
    product_grades_list = p.productgrade_set.all()
    if request.method == "POST":
        form = ProductInstanceForm(product_grades_list,request.POST)
        if form.is_valid():
            inst_prod = ProductInstance(weight = form.cleaned_data['weight'], grade = form.cleaned_data['grade'])
            inst_prod.save()
            logger.info(log_username(request) + " добавил в список: {0}, вес: {1}".format(inst_prod.grade,inst_prod.weight))
            return redirect('home')
    else:
        form = ProductInstanceForm(product_grades_list)
    return render(request,
        'app/product_grades.html',
        {
...
def edit_product(request,instance_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            instance.save()
            logger.info(log_username(request) + " внёс изменения в список. Изменил: Сорт: {0} -> {1}, Вес: {2} -> {3}".format(cur_grade,instance.grade,cur_weight,instance.weight))
            return redirect('home')
    else:
        form = ProductInstanceForm(product_grades_list, initial={'weight': instance.weight, 'grade': cur_grade})
    return render(request,
...
def delete_product(request,instance_id):
    instance = ProductInstance.objects.get(id = instance_id)
    instance.delete()
    logger.info(log_username(request) + " удалил из списка: {0}".format(instance.grade))
    return redirect('home')
...
def result(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            all_calories = sum([(i.weight * i.grade.calories) / 100 for i in ProductInstance.objects.all()])
            height = int(result.height)
            weight = int(result.weight)
            if result.gender == 'M':
                days_number = int(all_calories / (10 * weight + 6.25 * height - 5 * 30 + 5))
            else:
                days_number = int(all_calories / (10 * weight + 6.25 * height - 5 * 30 - 161))
            logger.info(log_username(request) + " получил результат рассчётов: {0}".format(days_number))        
    else:
...
def registration(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = auth.authenticate(username=user.username, password=my_password)
            auth.login(request, user)
            logger.info(log_username(request) + " зарегистрировался, пароль: {0}".format(my_password))
            return redirect('home')
    else:
